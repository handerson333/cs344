...
void play(){
	...
	while(1){     
		...
		i=0;		
		printf("POSSIBLE CONNECTIONS:");	//start possible connections
		while(i < current_room.num_connections - 1){//for all connections except the last at this room
			printf(" %s,", current_room.connections[i]->name);//print current connection
			i++;
		}
		printf(" %s.\n", current_room.connections[i]->name);	//finish printing connections with a period
